...
class Message {
  ...
  final String? subject; // Email subject (for Gmail)
...
class _MessageReaderPageState extends State<MessageReaderPage> {
  ...
  Future<void> _loadAllMessages() async {
    ...
    try {
      ...
      // Load Gmail emails - only if already signed in (non-blocking)
      List<Message> gmailList = [];
      try {
        // Only try silent sign-in - don't block with dialog
        final user = await GmailAuthService.silentSignIn();

        // If user is signed in, fetch emails
        if (user != null) {
          final userEmail = user.email;
          setState(() {
            gmailConnected = true;
            gmailUserEmail = userEmail;
          });
          // Fetch all emails (not just unread) - limit to 200 emails for performance
          final emails = await GmailService.fetchAllEmails(
            pageSize: 50,
            maxTotalResults: 200,
          );
          gmailList = emails.map((email) {
            return Message(
              id: 'gmail_${email.id}',
              address: email.from,
        ...
        // Continue without Gmail if there's an error
      }
                ...
                child: GestureDetector(
                    ...
                    if (gmailConnected) {
                      ...
                      final shouldSignOut = await showDialog<bool>(
                        ...
                        builder: (context) => AlertDialog(
                          ...
                          content: Text('Signed in as: $gmailUserEmail'),
                          actions: [
                            TextButton(
                      ...
                      try {
                        ...
                        final user = await GmailAuthService.signIn().timeout(
                        ...
                        if (user != null && mounted) {
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                              content: Text('Gmail sign-in cancelled'),
                              backgroundColor: Colors.orange,
                            ),
                          );
                        }
                  ...
                  child: Container(
                    ...
                    child: Row(
                      ...
                      children: [
                        Icon(
                          gmailConnected ? Icons.mail : Icons.mail_outline,
                          color: gmailConnected ? Colors.red : Colors.grey,
                          size: 16,
                        ),
                        const SizedBox(width: 6),
                        Text(
                          gmailConnected ? 'Gmail' : 'Sign In',
                          style: TextStyle(
                            color: gmailConnected ? Colors.red : Colors.grey,
                            fontSize: 12,
                            fontWeight: FontWeight.bold,
